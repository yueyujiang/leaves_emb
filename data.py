...
class data(Dataset):
    def __init__(self, args, calculate_distance_matrix=False):
        ...
        if not backbone_seq_file:
            tree_label = [i.label for i in tree.traverse_leaves()]
            s = [f">{l}\nACGT\n" for l in tree_label]
            with open(f'{args.model_dir}/fake_seq.fa', 'w') as f:
                f.write("".join(s))
            backbone_seq_file = f'{args.model_dir}/fake_seq.fa'
