...
class model(LightningModule):
    def __init__(self, args):
        ...
        self.hparams.c = 1.0
        species = [i.label for i in ts.read_tree_newick(args.backbone_tree_file).traverse_leaves()]
        if args.load_emb is None:
            self.embs = nn.Parameter(torch.rand(len(species), args.embedding_size) * 1e-7, requires_grad=True)
            # self.embs = self.embs * 1e-6
        else:
            embs = torch.load(args.load_emb)
            self.embs = nn.Parameter([embs[i] for i in species])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        encoding = torch.stack([self.embs[self.species.index(i)] for i in x], dim=0)
        # if self.hparams.distance_mode == 'hyperbolic':
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.distance_mode == 'hyperbolic':
            ...
            if (self.current_epoch + 1) % 10 == 0 and \
                ...
                lr = self.hparams.clr * 0.95 ** (self.current_epoch//200)
                self.current_clr = lr
            else:
                lr = 0
            # lr = 0
    ...
    def training_epoch_end(
        ...
        if self.current_epoch % 100 == 0:
            ...
            torch.save(self.embs, f"{self.hparams.model_dir}/embs.pt")
            torch.save(self.species, f"{self.hparams.model_dir}/species.pt")
            try:
                os.remove(f"epoch={self.current_epoch-100}.ckpt")
            except:
                pass
    ...
    def optimizer_step(
        ...
        if (self.trainer.current_epoch + 1) % self.hparams.lr_update_freq == 0:
